...
const Orders = ({user,nav,setNav}) => {
  ...
  useEffect(()=>{
    axios.get(`https://mychoice-server.vercel.app/mychoice/users/orders/${user}`)
    .then((res)=>{
      setIds(res.data);
    })
    .catch((err)=>{
  ...
  useEffect(()=>{
    axios.post("https://mychoice-server.vercel.app/mychoice/orders/list/ids",{ids})
    .then((res)=>{
  ...
  const cancel=(id)=>{
    ...
    setIds(updatedIds)
    axios.delete(`https://mychoice-server.vercel.app/mychoice/orders/remove/id/${id}`)
    .then((res)=>{
